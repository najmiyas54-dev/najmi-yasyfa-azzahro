...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Edit {{ ucfirst($category) }}</h1>

    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('admin.categories.update', [$category, $article->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div>
                        @if(filter_var($article->image_path, FILTER_VALIDATE_URL))
                            <img src="{{ $article->image_path }}" class="img-thumbnail" style="max-width: 200px;">
                        @else
                            <img src="{{ asset('storage/' . $article->image_path) }}" class="img-thumbnail" style="max-width: 200px;">
                        @endif
                    </div>
                </div>
                @endif
